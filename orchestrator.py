...
class AgentOrchestrator:
    ...
    def process_query(self, query: str, session_id: str = "default", user_location: Optional[Tuple[float, float]] = None, nearby_context: Optional[list] = None, walk_to_stop: Optional[dict] = None) -> str:
        ...
        try:
            ...
            # Use GPS location as route origin fallback
            if user_location and not origin_coords and router_output.primary_intent in ("find_route", "get_route"):
                origin_coords = (user_location[1], user_location[0])  # (lon, lat) format for ORS
                print(f"  📍 Using GPS location as route origin: {user_location}")
            ...
            # Inject nearby places into specialist_results so the synthesizer
            # can reference them naturally (the router never sees this data)
            if nearby_context:
                items_text = []
                for r in nearby_context:
                    entry = f"{r['name']} ({r['type']}, {r['distance_meters']}m away)"
                    if r.get("function"):
                        entry += f" - {r['function']}"
                    if r.get("lines"):
                        lines = r["lines"]
                        entry += f" - Lines: {', '.join(lines) if isinstance(lines, list) else lines}"
                    if r.get("poi_type"):
                        entry += f" - {r['poi_type']}"
                    items_text.append(entry)

                specialist_results["user_nearby"] = {
                    "success": True,
                    "note": (
                        "The user has shared their GPS location. These are places NEAR them "
                        "(they are not at these places, just nearby). Always say 'near' not 'from' "
                        "when referencing the user's position relative to these places. "
                        "For restaurants or POIs, say 'Near you there is...'. "
                        "Do NOT list all of these. Only mention the ones relevant to the user's question."
                    ),
                    "nearby_places": items_text
                }
                print(f"  📍 Nearby context: {len(items_text)} places passed to synthesizer")

            # Prepend walking step into Neo4j route data so the synthesizer
            # describes it naturally as part of the route (not as a separate instruction)
            if walk_to_stop and "neo4j" in specialist_results:
                neo4j_data = specialist_results["neo4j"]
                route = neo4j_data.get("route")
                if route and route.get("segments"):
                    walking_segment = {
                        "type": "walking",
                        "from": "Your location",
                        "to": walk_to_stop["stop_name"],
                        "distance_meters": walk_to_stop["walking_distance_meters"],
                        "walk_time_minutes": max(1, round(walk_to_stop["walking_duration_seconds"] / 60)),
                        "note": "Walk from your current GPS location to the nearest stop"
                    }
                    # Only prepend if the first segment isn't already a walk from user's location
                    first = route["segments"][0]
                    if not (first.get("type") == "walking" and first.get("from") == "Your location"):
                        route["segments"].insert(0, walking_segment)
                        route["estimated_duration_minutes"] = route.get("estimated_duration_minutes", 0) + walking_segment["walk_time_minutes"]
                        route["estimated_duration_text"] = f"{route['estimated_duration_minutes']} min"
                        print(f"  🚶 Prepended walking step: {walk_to_stop['walking_distance']} ({walk_to_stop['walking_duration']}) to {walk_to_stop['stop_name']}")
    ...
    def _execute_parallel(
        ...
        # Auto-trigger FIWARE if Neo4j returned fiware_query (e.g. mensa_menu) or fiware_type
        if "fiware" not in results:
            fiware_entity_id = self._extract_fiware_entity_id(results.get("neo4j"))
            if fiware_entity_id:
                results["fiware"] = self._auto_fiware_by_entity_id(fiware_entity_id)
            else:
    ...
    def _extract_fiware_entity_id(self, neo4j_result: Dict[str, Any]) -> Optional[str]:
        """Extract a direct FIWARE entity ID from Neo4j result (e.g. from find_places mensa_menu)."""
        if not neo4j_result or not neo4j_result.get("success"):
            return None

        # Check fiware_query (returned by find_places with query_type=mensa_menu)
        fiware_query = neo4j_result.get("fiware_query")
        if isinstance(fiware_query, dict) and fiware_query.get("entity_id"):
            return fiware_query["entity_id"]

        return None

    def _auto_fiware_by_entity_id(self, entity_id: str) -> Dict[str, Any]:
        """Fetch a FIWARE entity directly by ID (more reliable than type query)."""
        self._log(f"      🍽️ Auto-fetching FIWARE entity by ID: '{entity_id}'")
        try:
            result = self.fiware_client.get_entity_by_id(entity_id)
            if result.get("success"):
                entity = result.get("entity", {})
                self._log(f"      ✅ FIWARE entity fetched: {entity.get('id', 'unknown')}")
                return {
                    "success": True,
                    "entities": [entity],
                    "count": 1,
                ...
                self._log(f"      ❌ FIWARE entity fetch failed: {result.get('error')}")
                return result
        except Exception as e:
            self._log(f"      ❌ FIWARE entity fetch failed: {e}")
            return {"success": False, "error": str(e)}
