...
class Neo4jTransitGraph:
    ...
    def find_nearby_all(self, lat: float, lon: float, radius_meters: int = 500, limit: int = 10) -> Dict:
        """Find nearby nodes of all types (Building, Stop, POI, Landmark) sorted by distance."""
        self._log(f"[NEO4J] find_nearby_all: {lat}, {lon}, radius={radius_meters}m")
        with self.driver.session(database=self.database) as session:
            query = """
                MATCH (b:Building)
                WHERE b.latitude IS NOT NULL AND b.longitude IS NOT NULL
                WITH 'Building' as type, b.name as name,
                     b.latitude as latitude, b.longitude as longitude,
                     b.id as id, b.function as extra,
                     null as lines, null as poi_type,
                     point.distance(
                         point({latitude: $lat, longitude: $lon}),
                         point({latitude: b.latitude, longitude: b.longitude})
                     ) as distance
                ...
                WITH 'Stop' as type, s.name as name,
                     coalesce(s.latitude, s.lat) as latitude,
                     coalesce(s.longitude, s.lon) as longitude,
                     s.id as id, null as extra,
                     s.lines as lines, null as poi_type,
                     point.distance(
                         point({latitude: $lat, longitude: $lon}),
                         point({latitude: coalesce(s.latitude, s.lat), longitude: coalesce(s.longitude, s.lon)})
                     ) as distance
                WHERE distance <= $radius
                ...
                WITH 'POI' as type, p.name as name,
                     p.latitude as latitude, p.longitude as longitude,
                     null as id, null as extra,
                     null as lines, p.type as poi_type,
                     point.distance(
                         point({latitude: $lat, longitude: $lon}),
                         point({latitude: p.latitude, longitude: p.longitude})
                     ) as distance
                ...
                WITH 'Landmark' as type, l.name as name,
                     l.latitude as latitude, l.longitude as longitude,
                     null as id, l.description as extra,
                     null as lines, null as poi_type,
                     point.distance(
                         point({latitude: $lat, longitude: $lon}),
                ...
                RETURN type, name, latitude, longitude, id, extra, lines, poi_type,
                       round(distance) as distance_meters
                ORDER BY distance_meters
                LIMIT $limit
            """
            result = session.run(query, lat=lat, lon=lon, radius=radius_meters, limit=limit)
            items = []
            for record in result:
                item = {
                    "type": record["type"],
                ...
                if record["id"]:
                    item["id"] = record["id"]
                if record["extra"]:
                    item["function"] = record["extra"]
                if record["lines"]:
                    item["lines"] = record["lines"]
                if record["poi_type"]:
                    item["poi_type"] = record["poi_type"]
                items.append(item)
            return {
                "success": True,
