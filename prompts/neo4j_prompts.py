...
NEO4J_FUNCTIONS = {
    ...
    "find_places": {
        "description": "Generic place finder. Use query_type='mensa_menu' to fetch today's mensa menu via FIWARE.",
        "parameters": {
            "query_type": "'mensa_menu' for today's menu, 'search' for general search",
            "search_term": "Search text (optional, for query_type='search')"
        },
        "examples": [
            "What's the mensa menu today?",
            "Today's menu at mensa",
            "Show me the menu",
            "What's for lunch?"
        ]
    },
...
**Mensa menu queries** → find_places(query_type="mensa_menu")
- "What's the mensa menu?" → find_places(query_type="mensa_menu")
- "Today's menu" → find_places(query_type="mensa_menu")
- "What's for lunch?" → find_places(query_type="mensa_menu")
- "Show me the menu" → find_places(query_type="mensa_menu")
- "Can you share the menu?" → find_places(query_type="mensa_menu")
...
## COMPOUND QUERY HANDLING — CRITICAL!

You can only return ONE function call. When the user asks a compound/multi-part question,
you MUST pick the single function that answers the MOST important or specific part.

### Rules for compound queries:

1. **"Where is X?" + "Is X close/near to Y?"** → Pick `check_proximity(location1="X", location2="Y")`
   - check_proximity returns BOTH location names AND the distance/walk time between them
   - The synthesizer can answer "where is X?" from the location data in the proximity result
   - Do NOT pick find_any_location — it would answer "where is X?" but miss the distance part entirely

2. **"Where is X?" + "What's the menu?"** → Pick `find_places(query_type="mensa_menu")`
   - find_places returns the mensa location AND triggers FIWARE menu fetch
   - Covers both parts of the query

3. **"Where is X?" + "How far is X from Y?"** → Pick `get_distance_between_locations(location1="X", location2="Y")`
   - Same as check_proximity (it's an alias) — returns locations + distance

4. **"Where is X?" + "What's nearby?"** → Pick `get_nearby_buildings(building_id="X")` or `find_places_near_building(building_id="X")`
   - These return the building AND its neighbors

5. **"Where is X?" + "What tram goes there?"** → Pick `get_nearest_tram_from_building(building_id="X")`
   - Returns the building reference AND nearest transit

### General principle:
- **Always prefer the MORE SPECIFIC function** over a generic one
- `check_proximity` > `find_any_location` when distance/closeness is asked
- `get_distance_between_locations` > `find_any_location` when "how far" is asked
- `find_places_near_building` > `find_any_location` when "what's near X" is asked
- `find_places(query_type="mensa_menu")` > `find_any_location` when menu is asked
- When in doubt about compound queries: pick the function that answers the part the user would be MOST disappointed to NOT get answered

### Common compound patterns from the router:

The router may classify these as `compound_query` with sub_intents like `['get_location_info', 'get_location_info']`.
When you see `compound_query` or multiple sub_intents:
- Look at ALL entities (location, building_name, poi_name, origin, destination)
- Identify if there are TWO locations being compared → proximity/distance function
- Identify if there's a location + a "what" question → specific info function
...
NEO4J_FEW_SHOT_EXAMPLES = [
    ...
    },
    {
        ...
        "expected_output": {
            ...
            "parameters": {
                "query_type": "mensa_menu"
            },
            "confidence": 0.95,
            "reasoning": "Menu query → use find_places with query_type=mensa_menu to get the FIWARE entity ID for today's menu"
        }
    },
    {
        "user_query": "Can you share today's menu?",
        "router_output": {
            "primary_intent": "get_location_info",
            "entities": {"location": "mensa", "poi_name": "mensa", "time_constraint": "today"}
        },
        "expected_output": {
            "function_name": "find_places",
            "parameters": {
            ...
            "confidence": 0.92,
            "reasoning": "Follow-up menu query about mensa → find_places with query_type=mensa_menu"
        }
    },
    {
        "user_query": "Where is mensa? Is it close to Building 3?",
        "router_output": {
            "primary_intent": "compound_query",
            "sub_intents": ["get_location_info", "get_location_info"],
            "entities": {
                "location": "Mensa",
                "building_name": "Building 03",
                "poi_name": "Mensa"
        ...
        "expected_output": {
            ...
            "confidence": 0.95,
            "reasoning": "Compound query: 'where is X' + 'is it close to Y' → check_proximity answers BOTH parts (returns location names AND distance/walk time)"
        }
    },
    {
        "user_query": "How far is the library from Building 10? Where is the library?",
        "router_output": {
            "primary_intent": "compound_query",
            "sub_intents": ["get_location_info", "get_location_info"],
            "entities": {
        ...
        "expected_output": {
            "function_name": "get_distance_between_locations",
            "parameters": {
                "location1": "library",
                "location2": "Building 10"
            },
            "confidence": 0.95,
            "reasoning": "Compound query: 'how far' + 'where is' → get_distance_between_locations returns both locations AND distance"
        }
    },
    {
        "user_query": "Where is Building 05? Are there restaurants nearby?",
        "router_output": {
            "primary_intent": "compound_query",
            "sub_intents": ["get_location_info", "get_location_info"],
            "entities": {
        ...
        "expected_output": {
            ...
            "confidence": 0.93,
            "reasoning": "Compound query: 'where is building' + 'restaurants nearby' → find_places_near_building covers both (returns building reference + nearby restaurants with distances)"
        }
    },
    {
        "user_query": "Where is the mensa and what's the menu today?",
        "router_output": {
            "primary_intent": "compound_query",
            "sub_intents": ["get_location_info", "get_location_info"],
            "entities": {
        ...
        "expected_output": {
            ...
            "confidence": 0.93,
            "reasoning": "Compound query: 'where is mensa' + 'what's the menu' → find_places(mensa_menu) returns mensa location AND triggers FIWARE menu fetch"
        }
    },
    {
        "user_query": "Is Building 05 near Building 06?",
        "router_output": {
            "primary_intent": "get_location_info",
            "entities": {
                "location": "Building 05",
        ...
        "expected_output": {
            "function_name": "check_proximity",
            "parameters": {
                "location1": "Building 05",
                "location2": "Building 06"
            },
            "confidence": 0.95,
            "reasoning": "Proximity question between two locations → check_proximity"
        }
    },
    {
        "user_query": "Where is Izgaram and how far is it from campus?",
        "router_output": {
            "primary_intent": "compound_query",
            "sub_intents": ["get_location_info", "get_location_info"],
            "entities": {
        ...
        "expected_output": {
            ...
            "confidence": 0.88,
            "reasoning": "Compound query: 'where is X' + 'how far from campus' → get_distance_between_locations (using mensa as campus reference point). Returns both locations + distance."
        }
    }
]
